...
/**
 * Description of Sales_person_model
 *
...
class Sales_person_model extends CI_Model
{
    ...
    function read_biodata_by_ktp($ktp_no)
    {
        $query = "SELECT a.* , b.name as blacklist_name
                    FROM deptstore.salesperson_biodata a
                    left join master_salesperson_blacklist b on b.no = a.blacklist_no
                    WHERE a.ktp_no = '$ktp_no' ";
         
        $execute = $this->db->query($query);
        $data_result = $execute->result();
        return $this->format_output_select($data_result);
    }

    function save_sales_person_biodata($data)
    {
        $error = array();
        $ktp_no = addSingleQuotes($data['ktp_no']);
        $name = addSingleQuotes($data['name']);
        $gender = addSingleQuotes($data['gender']);
        $birth_city = addSingleQuotes($data['birth_city']);
        $birth_date = addSingleQuotes($data['birth_date']);
        $address_street = addSingleQuotes($data['address_street']);
        $address_city = addSingleQuotes($data['address_city']);
        $address_province = addSingleQuotes($data['address_province']);
        $address_postal_code = addSingleQuotes($data['address_postal_code']);
        $phone = addSingleQuotes($data['phone']);
        $email = addSingleQuotes($data['email']);
        $religion = addSingleQuotes($data['religion']);
        $education = addSingleQuotes($data['education']);
        $experience = addSingleQuotes($data['experience']);
        $is_merried = addSingleQuotes($data['is_merried']);

        $this->db->trans_begin();

        $sqlInsert = "INSERT INTO deptstore.salesperson_biodata 
            (ktp_no, name, gender, birth_city, birth_date, address_street, address_city, 
            address_province, address_postal_code, phone, email, religion, experience, is_merried) 
            VALUES
            ($ktp_no, $name, $gender, $birth_city, $birth_date, $address_street, $address_city,
            $address_province, $address_postal_code, $phone, $email, $religion, $experience, $is_merried )
            ON DUPLICATE KEY UPDATE 
            ktp_no=$ktp_no, name= $name, gender=$gender, birth_city=$birth_city, birth_date=$birth_date, address_street=$address_street, address_city=$address_city, 
            address_province=$address_province, address_postal_code=$address_postal_code, phone=$phone , email=$email, religion=$religion, experience=$experience, is_merried=$is_merried";
...
//    return $sqlInsert;
        ...
        // $test2 = $this->db->trans_status();
        // $test3 = $this->db->insert_id();
        // return $test;
...
// --------------------------------------------------- END ------------------------------------------------------------------------------------------
    ...
    function get_user_data($user_id, $user_password)
    {
        $query = "SELECT * FROM deptstore.user a WHERE a.email = $user_id OR a.phone = $user_id AND a.password = $user_password ";
        ...
        if (!empty($user)) {
            foreach ($user as $obj) {
                // $obj->MenuList = $this->getAccessMenu($obj->user_id);
...
//     function get_user_data($user_id, $user_password)
//     {
//         $query = "SELECT * FROM kd_deptstore.user a WHERE a.email = $user_id OR a.phone = $user_id AND a.password = $user_password ";
...
//     }
    ...
    function get_group($group_no) {
        $query = "SELECT * 
                  FROM kd_deptstore.master_user_group
                  WHERE no IN($group_no)
    ...
    function get_menu($menu_list) 
    {
        $query = "SELECT group_no, no, parent_no, name, icon, url, sidebar
                      FROM kd_deptstore.master_menu
                      WHERE no IN($menu_list) 
                        AND parent_no = 0";
        ...
        if(!empty($menu)) {
            foreach($menu as $obj) {
                $obj->Child = $this->get_child_menu($obj->no);
            }
        }
    ...
    function get_child_menu($parent_no) {
        $query = "SELECT group_no, no, parent_no, name, icon, url, sidebar
                      FROM kd_deptstore.master_menu
                      WHERE parent_no IN($parent_no) 
                      ORDER BY no ASC";
    ...
    // {
    //     $sql = "SELECT *
    //               FROM kawandigital.master_user_api a
    //               LEFT JOIN kawandigital.master_app b ON b.app_id = a.app_id
    ...
    public function format_output_select_old($data)
    {
        $output = array();
        $output['total_records'] = count($data);
        $output['data'] = $data;
